...
const User = mongoose.model('User', new mongoose.Schema({
    ...
    roles: { type: [String]}//, enum: ["Administrator", "Editor", "Reader"]}
...
let users= [
    {email: "rob@dci.de", password: "pw1", roles: ['Admin', 'Teacher']},
     {email: "ham@dci.de", password: "pw1", roles: ['Admin', 'Reader']},
     {email: "moj@dci.de", password: "pw1", roles: ['Admin', 'Editor']}
    ]


app.get("/users/seed",(req, res)=>{
    console.log("/users/seed called")
    console.log(req.body)//when u have a form 
    let usersHash =users.map(user =>{
     ...
     User.insertMany(usersHash).then(users=>{
         console.log(users)
         res.send(users)
